...
describe("ArtistAttribution", function () {
  ...
  const deployAndTrainArtist = ({
    ...
    async function deployContractsAndTrainArtist() {
      ...
      await expect(artistContract.connect(artist1Account).CreateArtist('artist1', '123', {
        value: DEFAULT_ARTIST_COST,
      })).not.to.be.reverted

      await expect(artistContract.connect(owner).ArtistComplete('artist1')).not.to.be.reverted
      return ret
    }
  ...
  const deployAndPostImage = ({
    ...
    async function deployContractsAndPostImage() {
      ...
      await expect(artistContract.connect(customerAccount).CreateImage('artist1', 'hello world', {
  ...
  //describe("Image", function () {
    ...
    //   await expect(artistContract.connect(customerAccount).CreateImage('artist1', 'hello world')).to.be.revertedWith(
    ...
    //   const tx = await artistContract.connect(customerAccount).CreateImage('artist1', 'hello world', {
    //     value: DEFAULT_IMAGE_COST,
    //   })
