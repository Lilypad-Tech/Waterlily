...
contract ArtistAttribution is LilypadCallerInterface, Ownable {
    ...
    function getArtists() public view returns (mapping (string => Artist) storage) {
        return artists;
    }

    function getImageIDs() public view returns (uint[] storage) {
        return imageIDs;
    }

    function getImage(uint id) public view returns (StableDiffusionImage storage) {
        return images[id];
    }
